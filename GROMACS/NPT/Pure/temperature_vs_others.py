...
def plot_data(new_data):
    ...
    if 'Temperature (K)' in new_data.columns and 'Density (kg/m³)' in new_data.columns:
        ...
        plt.grid(True)
        plt.tight_layout()
        # plt.savefig('temperature_vs_density.png', dpi=300)
        plt.show()
        plt.close()
    else :
    ...
    if 'Temperature (K)' in new_data.columns and 'Volume (nm³)' in new_data.columns:
        ...
        plt.grid(True)
        plt.tight_layout()
        # plt.savefig('temperature_vs_volume.png', dpi=300)
        plt.show()
        plt.close()
    else :
    ...
    if 'Temperature (K)' in new_data.columns and 'Heat Capacity (kJ/(mol K))' in new_data.columns:
        ...
        plt.grid(True)
        plt.tight_layout()
        # plt.savefig('temperature_vs_heat_capacity.png', dpi=300)
        plt.show()
        plt.close()
    else :
...
filepath = '/Users/liming/Desktop/AutoPloyPlot/GROMACS/NPT/Pure/density_temp01.xvg'
